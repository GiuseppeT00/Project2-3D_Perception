...
def fuse_latent_features(self, pca: Union[PCA, Isomap], latent_features: list) -> torch.Tensor:
...
def overloaded_forward_single_backbone(self, xs: list, pca: Union[PCA, Isomap]):
    input_shape = xs[0].shape[-2:]
    xs = [x.detach().cpu() for x in xs]
    features = self.fuse_latent_features(pca=pca, latent_features=xs)
    features = features.to(torch.device('cuda')).requires_grad_()
    features = self.backbone(features)
    x1 = self.classifier(features['out'])
    x1 = F.interpolate(x1, size=input_shape, mode='bilinear', align_corners=False)
    if self.aux_classifier is not None:
        x2 = self.aux_classifier(features['aux'])
        x2 = F.interpolate(x2, size=input_shape, mode='bilinear', align_corners=False)
    return {'out': x1, 'aux': x2} if self.aux_classifier is not None else x1


def overloaded_forward_multi_backbone(self, xs: list, pca: Union[PCA, Isomap]):
    pass


def get_model(model_name: str, backbone: str,
              multi_modal: bool, multi_backbone: bool,
              classes: int, old_error: bool = False) -> nn.Module:
    models = {
...
if __name__ == '__main__':
    ...
    pca = Isomap(n_components=1)
    imgs_orig = [torch.rand((2, 3, 5, 5), dtype=torch.float32) for i in range(4)]
